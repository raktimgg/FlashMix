...
def main(args):
    ...
    batch_size=300
    dset = CommonLoader(pickle_loc=pickle_loc, dataset_name=dataset_name, base_ds=base_ds, do_augment=True, train=True)
    ...
    local_feature_path = 'src/utils/local_features_vReLoc_0.npy'
    gt_pose_path = 'src/utils/gt_pose_vReLoc_0.npy'
    xyz_path = 'src/utils/xyz_vReLoc_0.npy'
    ...
    if os.path.exists(local_feature_path):
        ...
        with torch.inference_mode():
            ...
            k = 0
            N_max = 512
            for i in range(6): # increasing the dataset size by augmenting
    ...
    if do_validation:
        ...
        local_feature_val_path = 'src/utils/local_features_vReLoc_val.npy'
        gt_pose_val_path = 'src/utils/gt_pose_vReLoc_val.npy'
        xyz_val_path = 'src/utils/xyz_vReLoc_val.npy'
    ...
    for e in range(MAX_EPOCH):
        ...
        save_loc = args['training']['save_folder'] + '/vReLoc_regressor_' + str(exp_num) + '.pth'
        ...
        if do_validation:
            ...
            for i in range(len(error_r_list)):
                if error_t_list[i] < 0.25 and error_r_list[i] < 5:
                    success+=1
            success_rate = success/len(error_r_list)*100

            print(f'Avg. Validation Loss {epoch_val_loss_avg:.2f}, Translation Error {np.median(error_t_list):.2f}, Rotation Error {np.median(error_r_list):.2f}, Success Rate {success_rate:.2f}')

        print(' ')
        if do_wandb:
            if do_validation:
                wandb.log({"Current LR":current_lr, "Training loss": epoch_loss_avg,"Validation loss": epoch_val_loss_avg, "Translation Error":np.median(error_t_list), "Rotation Error":np.median(error_r_list)})
            else:
