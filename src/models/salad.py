...
def log_optimal_transport(scores: torch.Tensor, alpha: torch.Tensor, iters: int) -> torch.Tensor:
    ...
    bins = alpha.repeat(b, 1, n)
    alpha = alpha.repeat(b, 1, 1)
    ...
    log_mu = torch.cat([norm.repeat(m), bs.log()[None] + norm])
    log_nu = norm.repeat(n)
    log_mu, log_nu = log_mu[None].repeat(b, 1), log_nu[None].repeat(b, 1)
