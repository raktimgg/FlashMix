...
class UBlock(nn.Module):
    ...
    def forward(self, inp, xyz, batch):
        ...
        if len(self.nPlanes) > 1:
            ...
            output_decoder, output_list = self.u(output_decoder, xyz_next, batch_next.long())

            # upsample
            output_decoder = self.deconv(output_decoder)
            output = output.replace_feature(torch.cat((identity.features, output_decoder.features), dim=1))
            output = self.blocks_tail(output)
            output_list.append(output.features)
        else:
            output_list = []
            output_list.append(output.features)
...
class Semantic(nn.Module):
    ...
    def forward(self, input, xyz, batch):
        ...
        output, output_list = self.unet(output, xyz, batch)
        output2 = copy.deepcopy(output.features)
        output = self.output_layer(output)
        output_list.append(output.features)
        # print(output_list[0].shape,output_list[1].shape,output_list[2].shape,output_list[3].shape)


        # # ### semantic segmentation
        # semantic_scores = self.linear(output.features)   # (N, nClass), float
        # return semantic_scores
        # return torch.concatenate([output.features,output2],axis=-1)
